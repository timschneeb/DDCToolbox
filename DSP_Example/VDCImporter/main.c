...
{
	int i;
	char filename[] = "Beyerdynamic DT770-80-4.vdc";
	FILE *binary = fopen(filename, "rb");
	struct stat st;
	char *textString;
	if (stat(filename, &st) == 0)
	{
		textString = malloc(st.st_size);
		fread(textString, sizeof(char), st.st_size, binary);
		fclose(binary);
	}
	else
	{
	...
	free(textString);
	printf("48k DDC SOS Matrix:\n");
	for (i = 0; i < sosCount; i++)
		printf("%.17g %.17g %.17g 1.0 %.17g %.17g\n", df48[i]->b0, df48[i]->b1, df48[i]->b2, df48[i]->a1, df48[i]->a2);
	printf("---SOS print end---\n");
	printf("---Print vdcprj---\n\n");
