...
QVector<QueryResult> AutoEQClient::query(QueryRequest request){
    ...
    if(net_request.lastError() != ""){
        qCritical() << "An error occurred (query): " << net_request.lastError();
        return results;
    }
...
HeadphoneMeasurement AutoEQClient::fetchDetails(QueryResult item){
    ...
    if(net_request.lastError() != ""){
        qCritical() << "An error occurred (fetchDetails): " << net_request.lastError();
        return hp;
    }
    ...
    foreach (const QJsonValue & v, array)
    {
        ...
        if(name.endsWith("ParametricEQ.txt")){
            ...
            if(net_request_eq.lastError() != ""){
                qCritical() << "An error occurred (fetchDetails/parameq): " << net_request.lastError();
            }
