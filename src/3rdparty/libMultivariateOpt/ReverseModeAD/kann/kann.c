...
static void kad_ext_collate(int n, kad_node_t **a, double **_x, double **_g, double **_c)
{
	int i, j, k, l, n_var;
	double *x, *g, *c;
	n_var = kad_size_var(n, a);
	x = *_x = (double*)realloc(*_x, n_var * sizeof(double));
	g = *_g = (double*)realloc(*_g, n_var * sizeof(double));
	c = *_c = (double*)realloc(*_c, kad_size_const(n, a) * sizeof(double));
	memset(g, 0, n_var * sizeof(double));
	for (i = j = k = 0; i < n; ++i) {
		kad_node_t *v = a[i];
		if (kad_is_var(v)) {
			l = kad_len(v);
			memcpy(&x[j], v->x, l * sizeof(double));
			free(v->x);
			v->x = &x[j];
			v->g = &g[j];
			j += l;
			...
			l = kad_len(v);
			memcpy(&c[k], v->x, l * sizeof(double));
			free(v->x);
			v->x = &c[k];
			k += l;
		}
...
static void kad_ext_sync(int n, kad_node_t **a, double *x, double *g, double *c)
{
	int i, j, k;
	for (i = j = k = 0; i < n; ++i) {
		kad_node_t *v = a[i];
...
int kann_feed_bind(kann_t *a, uint32_t ext_flag, int32_t ext_label, double **x)
{
	int i, k;
	if (x == 0) return 0;
	for (i = k = 0; i < a->n; ++i)
...
static double kann_cost_core(kann_t *a, int cost_label, int cal_grad)
{
	int i_cost;
	double cost;
...
void kann_rnn_start(kann_t *a)
{
	...
	for (i = 0; i < a->n; ++i) {
		...
		if (p->pre) { /* NB: BE CAREFUL of the interaction between kann_rnn_start() and kann_set_batch_size() */
			kad_node_t *q = p->pre;
			if (q->x) memcpy(p->x, q->x, kad_len(p) * sizeof(double));
			else memset(p->x, 0, kad_len(p) * sizeof(double));
...
void kann_rnn_end(kann_t *a)
{
	...
	for (i = 0; i < a->n; ++i)
		if (a->v[i]->pre && a->v[i]->pre->n_child > 0)
			a->v[i]->pre->x = (double*)calloc(kad_len(a->v[i]->pre), sizeof(double));
}
	...
	for (i = 0, *base = 0; i < ann->n; ++i) {
		...
		if (((p->op == 13 && (p->n_child == 2 || p->n_child == 3)) || (p->op == 22 && p->n_child == 2)) && p->n_d == 0) { /* ce_bin or ce_multi */
			...
			for (j = off = 0; j < m; ++j, off += n) {
				double t_sum = 0.0, t_min = 1.0, t_max = 0.0, x_max = 0.0, x_min = 1.0;
				int x_max_k = -1, t_max_k = -1;
				for (k = 0; k < n; ++k) {
					double xk = x->x[off+k], tk = t->x[off+k];
					t_sum += tk;
				...
				if (t_sum - 1.0 == 0 && t_min >= 0.0 && x_min >= 0.0 && x_max <= 1.0) {
					++(*base);
...
static void mt_kickoff(kann_t *a, int cost_label, int cal_grad, int eval_out)
{
	...
	for (i = k = 0; i < mt->n_threads; ++i) {
		...
		k += size;
		memcpy(mt->mt[i].a->x, a->x, n_var * sizeof(double));
		mt->mt[i].action = 1;
	}
	mt->n_idle = 0;
	pthread_cond_broadcast(&mt->cv);
...
double kann_cost(kann_t *a, int cost_label, int cal_grad)
{
	mtaux_t *mt = (mtaux_t*)a->mt;
	int i, j, B, k, n_var;
	double cost;
	...
	memset(a->g, 0, n_var * sizeof(double)); /* TODO: check if this is necessary when cal_grad is false */
	for (i = k = 0, cost = 0.0; i < mt->n_threads; ++i) {
		int size = (B - k) / (mt->n_threads - i);
		cost += mt->mt[i].cost * size / B;
		kad_saxpy(n_var, (double)size / B, mt->mt[i].a->g, a->g);
		k += size;
	}
	for (j = 0; j < a->n; ++j) { /* copy values back at recurrent nodes (needed by textgen; TODO: temporary solution) */
		...
		if (p->pre && p->n_d >= 2 && p->d[0] == B) {
			for (i = k = 0; i < mt->n_threads; ++i) {
				...
				memcpy(&p->x[k], q->x, kad_len(q) * sizeof(double));
				k += kad_len(q);
			}
		}
	}
		...
		if (p->ext_flag & KANN_F_OUT) {
			...
			for (i = 0; i < d0; ++i) {
				for (t = k = 0; t < mt->n_threads; ++t) { /* similar to the forward pass of kad_op_concat() */
					...
					int size = q->d[0] / d0;
					memcpy(&p->x[(i * B + k) * d1], &q->x[i * size * d1], size * d1 * sizeof(double));
					k += size;
				}
			}
		}
...
#else
void kann_mt(kann_t *ann, int n_threads, int max_batch_size) {}
double kann_cost(kann_t *a, int cost_label, int cal_grad) { return kann_cost_core(a, cost_label, cal_grad); }
...
void kann_save_fp(FILE *fp, kann_t *ann)
{
	...
	fwrite(ann->x, sizeof(double), kann_size_var(ann), fp);
	fwrite(ann->c, sizeof(double), kann_size_const(ann), fp);
}
...
kann_t *kann_load_fp(FILE *fp)
{
	...
	ann->x = (double*)malloc(n_var * sizeof(double));
	ann->g = (double*)calloc(n_var, sizeof(double));
	ann->c = (double*)malloc(n_const * sizeof(double));
	fread(ann->x, sizeof(double), n_var, fp);
	fread(ann->c, sizeof(double), n_const, fp);
...
kad_node_t *kann_new_leaf_array(int *offset, kad_node_p *par, uint8_t flag, double x0_01, int n_d, int32_t d[KAD_MAX_DIM])
{
	...
	len = kad_len(p);
	p->x = (double*)calloc(len, sizeof(double));
	if (p->n_d <= 1 || fabs(x0_01) > 0.0) {
		for (i = 0; i < len; ++i)
			p->x[i] = x0_01;
	} else {
		double sdev_inv;
		...
		for (i = 0; i < len; ++i)
			p->x[i] = (double)(kad_drand_normal(0) * sdev_inv);
	}
	if (off >= 0) par[off] = p, ++(*offset);
	return p;
}

kad_node_t *kann_new_leaf2(int *offset, kad_node_p *par, uint8_t flag, double x0_01, int n_d, ...)
{
...
kad_node_t *kann_layer_dense2(int *offset, kad_node_p *par, kad_node_t *in, int n1)
{
	...
	w = kann_new_leaf2(offset, par, KAD_VAR, 0.0, 2, n1, n0);
	b = kann_new_leaf2(offset, par, KAD_VAR, 0.0, 1, n1);
	return kad_add(kad_cmul(in, w), b);
}

kad_node_t *kann_layer_dropout2(int *offset, kad_node_p *par, kad_node_t *t, double r)
{
	kad_node_t *x[2], *cr;
...
kad_node_t *kann_layer_layernorm2(int *offset, kad_node_t **par, kad_node_t *in)
{
	...
	n0 = in->n_d >= 2? kad_len(in) / in->d[0] : kad_len(in);
	alpha = kann_new_leaf2(offset, par, KAD_VAR, 1.0, 1, n0);
	beta  = kann_new_leaf2(offset, par, KAD_VAR, 0.0, 1, n0);
...
kad_node_t *kann_layer_rnn2(int *offset, kad_node_t **par, kad_node_t *in, kad_node_t *h0, int rnn_flag)
{
	...
	u = kann_new_leaf2(offset, par, KAD_VAR, 0.0, 2, n1, n1);
	b = kann_new_leaf2(offset, par, KAD_VAR, 0.0, 1, n1);
...
kad_node_t *kann_new_leaf(uint8_t flag, double x0_01, int n_d, ...)
{
	int32_t i, d[KAD_MAX_DIM];
	va_list ap;
	va_start(ap, n_d); for (i = 0; i < n_d; ++i) d[i] = va_arg(ap, int); va_end(ap);
...
kad_node_t *kann_new_scalar(uint8_t flag, double x) { return kann_new_leaf(flag, x, 0); }
kad_node_t *kann_new_weight(int n_row, int n_col) { return kann_new_leaf(KAD_VAR, 0.0, 2, n_row, n_col); }
kad_node_t *kann_new_vec(int n, double x) { return kann_new_leaf(KAD_VAR, x, 1, n); }
kad_node_t *kann_new_bias(int n) { return kann_new_vec(n, 0.0); }
kad_node_t *kann_new_weight_conv2d(int n_out, int n_in, int k_row, int k_col) { return kann_new_leaf(KAD_VAR, 0.0, 4, n_out, n_in, k_row, k_col); }
kad_node_t *kann_new_weight_conv1d(int n_out, int n_in, int kernel_len) { return kann_new_leaf(KAD_VAR, 0.0, 3, n_out, n_in, kernel_len); }
...
kad_node_t *kann_layer_dropout(kad_node_t *t, double r) { return kann_layer_dropout2(0, 0, t, r); }
...
kad_node_t *kann_layer_rnn(kad_node_t *in, int n1, int rnn_flag)
{
	...
	h0->x = (double*)calloc(n1, sizeof(double));
...
kad_node_t *kann_layer_lstm(kad_node_t *in, int n1, int rnn_flag)
{
	...
	c0->x = (double*)calloc(n1, sizeof(double));
	...
	w = kann_new_weight(n1, n0);
	u = kann_new_weight(n1, n1);
	b = kann_new_vec(n1, 1.0); /* see Jozefowicz et al on using a large bias */
...
kad_node_t *kann_layer_mse_raw(kad_node_t *t, int n_out)
{
	kad_node_t *cost = 0, *truth = 0;
	truth = kad_feed(2, 1, n_out), truth->ext_flag |= KANN_F_TRUTH;
	cost = kad_mse(t, truth);
...
void kann_RMSprop(grad_optimizer *state, const double *g, double *t)
{
	...
	vh = _mm_set1_ps(state->param1);
	vd = _mm_set1_ps(state->param2);
	vd1 = _mm_set1_ps(1.0 - state->param2);
	vtiny = _mm_set1_ps(state->eps);
	for (i = 0; i < n4; i += 4) {
		vt = _mm_loadu_ps(&t[i]);
		vr = _mm_loadu_ps(&state->history[i]);
		vg = _mm_loadu_ps(&g[i]);
		vr = _mm_add_ps(_mm_mul_ps(vd1, _mm_mul_ps(vg, vg)), _mm_mul_ps(vd, vr));
		_mm_storeu_ps(&state->history[i], vr);
		tmp = _mm_sub_ps(vt, _mm_mul_ps(_mm_mul_ps(vh, _mm_rsqrt_ps(_mm_add_ps(vtiny, vr))), vg));
		_mm_storeu_ps(&t[i], tmp);
	}
	for (; i < n; ++i) {
		state->history[i] = (1.0 - state->param2) * g[i] * g[i] + state->param2 * state->history[i];
...
{
	state->n = n;
	state->history = (double*)malloc(n * sizeof(double));
	memset(state->history, 0, n * sizeof(double));
	state->param1 = rho;
	state->param2 = decay;
	state->changingParam1 = 1.0 - decay;
	state->eps = eps;
}
void freeRMSProp(grad_optimizer *state)
{
	free(state->history);
}
void kann_RMSprop(grad_optimizer *state, const double *g, double *t)
{
	...
	for (i = 0; i < state->n; ++i)
	{
		state->history[i] = state->param2 * state->history[i] + state->changingParam1 * g[i] * g[i];
...
int kann_train_fnn1(kann_t *ann, double lr, int mini_size, int max_epoch, int max_drop_streak, double frac_val, int n, double **_x, double **_y)
{
	int i, j, *shuf, n_train, n_val, n_in, n_out, n_var, n_const, drop_streak = 0, min_set = 0;
	double **x, **y, *x1, *y1, min_val_cost = FLT_MAX, *min_x, *min_c;
	...
	kann_InitRMSprop(&state, n_var, &lr, 0.9f, 1e-6f);
	...
	n_train = n - n_val;
	min_x = (double*)malloc(n_var * sizeof(double));
	min_c = (double*)malloc(n_const * sizeof(double));

	x1 = (double*)malloc(n_in  * mini_size * sizeof(double));
	y1 = (double*)malloc(n_out * mini_size * sizeof(double));
	...
	for (i = 0; i < max_epoch; ++i) {
		...
		while (n_proc < n_train) {
			...
			for (b = 0; b < ms; ++b) {
				memcpy(&x1[b*n_in],  x[shuf[n_proc+b]], n_in  * sizeof(double));
				memcpy(&y1[b*n_out], y[shuf[n_proc+b]], n_out * sizeof(double));
			}
			kann_set_batch_size(ann, ms);
			double cost = kann_cost(ann, 0, 1) * ms;
			//printf("%1.7f\n", cost);
			train_cost += cost;
		...
		while (n_proc < n_val) {
			...
			for (b = 0; b < ms; ++b) {
				memcpy(&x1[b*n_in],  x[n_train+n_proc+b], n_in  * sizeof(double));
				memcpy(&y1[b*n_out], y[n_train+n_proc+b], n_out * sizeof(double));
			}
			kann_set_batch_size(ann, ms);
		...
		if (i >= max_drop_streak && n_val > 0) {
			if (val_cost < min_val_cost) {
				min_set = 1;
				memcpy(min_x, ann->x, n_var * sizeof(double));
				memcpy(min_c, ann->c, n_const * sizeof(double));
				drop_streak = 0;
				min_val_cost = (double)val_cost;
			} else if (++drop_streak >= max_drop_streak)
				break;
		}
	}
	if (min_set) {
		memcpy(ann->x, min_x, n_var * sizeof(double));
		memcpy(ann->c, min_c, n_const * sizeof(double));
	}

	free(min_c); free(min_x); free(y1); free(x1); free(y); free(x); free(shuf);
	freeRMSProp(&state);
	return i;
}

double kann_cost_fnn1(kann_t *ann, int n, double **x, double **y)
{
	...
	while (n_proc < n) {
		...
		for (b = 0; b < ms; ++b) {
			memcpy(&x1[b*n_in],  x[n_proc+b], n_in  * sizeof(double));
			memcpy(&y1[b*n_out], y[n_proc+b], n_out * sizeof(double));
		}
		kann_set_batch_size(ann, ms);
...
const double *kann_apply1(kann_t *a, double *x)
{
	int i_out;
